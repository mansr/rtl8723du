...
static u32 halbtcoutsrc_Read_scbd_32bit(void *pBtcContext, u32* score_board_val)
{
	PBTC_COEXIST btc = (PBTC_COEXIST)pBtcContext;
	struct btc_coex_sta *coex_sta = &btc->coex_sta;
	const struct btc_chip_para *chip_para = btc->chip_para;
...
static void halbtcoutsrc_Write_scbd_32bit(void *pBtcContext, u32 bitpos, u8 state)
{
	PBTC_COEXIST btc = (PBTC_COEXIST)pBtcContext;
	struct btc_coex_sta *coex_sta = &btc->coex_sta;
	const struct btc_chip_para *chip_para = btc->chip_para;
...
static __maybe_unused void EXhalbtcoutsrc_dbg_control(PBTC_COEXIST pBtCoexist, u8 opCode, u8 opLen, u8 *pData)
{
	if (!halbtcoutsrc_IsBtCoexistAvailable(pBtCoexist))
		return;
